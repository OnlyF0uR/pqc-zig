...
pub const SIG_BYTE_LEN: u32 = 666;

// #define PQCLEAN_FALCONPADDED512_CLEAN_CRYPTO_ALGNAME          "Falcon-padded-512"
pub const ALG_NAME: []const u8 = "Falcon-padded-512";
...
pub fn crypto_sign_keypair(
    ...
    if (build_options.avx2) {
        ...
        return c.PQCLEAN_FALCONPADDED512_AVX2_crypto_sign_keypair(pk, sk);
    } else if (build_options.aarch64) {
        const c = @cImport({
            @cInclude("aarch64/api.h");
        });
        return c.PQCLEAN_FALCONPADDED512_AARCH64_crypto_sign_keypair(pk, sk);
    } else {
        const c = @cImport({
            @cInclude("clean/api.h");
        });
        return c.PQCLEAN_FALCONPADDED512_CLEAN_crypto_sign_keypair(pk, sk);
    }
}
...
pub fn crypto_sign_signature(
    ...
    if (build_options.avx2) {
        ...
        return c.PQCLEAN_FALCONPADDED512_AVX2_crypto_sign_signature(sig, siglen, m, mlen, sk);
    } else if (build_options.aarch64) {
        const c = @cImport({
            @cInclude("aarch64/api.h");
        });
        return c.PQCLEAN_FALCONPADDED512_AARCH64_crypto_sign_signature(sig, siglen, m, mlen, sk);
    } else {
        const c = @cImport({
            @cInclude("clean/api.h");
        });
        return c.PQCLEAN_FALCONPADDED512_CLEAN_crypto_sign_signature(sig, siglen, m, mlen, sk);
    }
}
...
pub fn crypto_sign_verify(
    ...
    if (build_options.avx2) {
        ...
        return c.PQCLEAN_FALCONPADDED512_AVX2_crypto_sign_verify(sig, siglen, m, mlen, pk);
    } else if (build_options.aarch64) {
        const c = @cImport({
            @cInclude("aarch64/api.h");
        });
        return c.PQCLEAN_FALCONPADDED512_AARCH64_crypto_sign_verify(sig, siglen, m, mlen, pk);
    } else {
        const c = @cImport({
            @cInclude("clean/api.h");
        });
        return c.PQCLEAN_FALCONPADDED512_CLEAN_crypto_sign_verify(sig, siglen, m, mlen, pk);
    }
}
...
pub fn crypto_sign(
    ...
    if (build_options.avx2) {
        ...
        return c.PQCLEAN_FALCONPADDED512_AVX2_crypto_sign(sm, smlen, m, mlen, sk);
    } else if (build_options.aarch64) {
        const c = @cImport({
            @cInclude("aarch64/api.h");
        });
        return c.PQCLEAN_FALCONPADDED512_AARCH64_crypto_sign(sm, smlen, m, mlen, sk);
    } else {
        const c = @cImport({
            @cInclude("clean/api.h");
        });
        return c.PQCLEAN_FALCONPADDED512_CLEAN_crypto_sign(sm, smlen, m, mlen, sk);
    }
}
...
pub fn crypto_sign_open(
    ...
    if (build_options.avx2) {
        ...
        return c.PQCLEAN_FALCONPADDED512_AVX2_crypto_sign_open(m, mlen, sm, smlen, pk);
    } else if (build_options.aarch64) {
        const c = @cImport({
            @cInclude("aarch64/api.h");
        });
        return c.PQCLEAN_FALCONPADDED512_AARCH64_crypto_sign_open(m, mlen, sm, smlen, pk);
    } else {
        const c = @cImport({
            @cInclude("clean/api.h");
        });
        return c.PQCLEAN_FALCONPADDED512_CLEAN_crypto_sign_open(m, mlen, sm, smlen, pk);
    }
}
...
test "byte lengths" {
    if (build_options.avx2) {
        ...
        try std.testing.expectEqual(SK_BYTE_LEN, c.PQCLEAN_FALCONPADDED512_CLEAN_CRYPTO_SECRETKEYBYTES);
        try std.testing.expectEqual(PK_BYTE_LEN, c.PQCLEAN_FALCONPADDED512_CLEAN_CRYPTO_PUBLICKEYBYTES);
        try std.testing.expectEqual(SIG_BYTE_LEN, c.PQCLEAN_FALCONPADDED512_CLEAN_CRYPTO_BYTES);
        try std.testing.expect(std.mem.eql(u8, ALG_NAME, c.PQCLEAN_FALCONPADDED512_CLEAN_CRYPTO_ALGNAME));
    } else if (build_options.aarch64) {
        const c = @cImport({
            @cInclude("aarch64/api.h");
        });
