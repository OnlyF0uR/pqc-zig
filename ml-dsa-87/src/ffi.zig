...
pub fn crypto_sign_keypair(
    ...
    if (build_options.avx2) {
        ...
        return c.PQCLEAN_MLDSA87_AVX2_crypto_sign_keypair(pk, sk);
    } else if (build_options.aarch64) {
        const c = @cImport({
            @cInclude("aarch64/api.h");
        });
        return c.PQCLEAN_MLDSA87_AARCH64_crypto_sign_keypair(pk, sk);
    } else {
        const c = @cImport({
            @cInclude("clean/api.h");
        });
        return c.PQCLEAN_MLDSA87_CLEAN_crypto_sign_keypair(pk, sk);
    }
}
...
pub fn crypto_sign_signature_ctx(
    ...
    if (build_options.avx2) {
        ...
        return c.PQCLEAN_MLDSA87_AVX2_crypto_sign_signature_ctx(sig, siglen, m, mlen, ctx, ctxlen, sk);
    } else if (build_options.aarch64) {
        const c = @cImport({
            @cInclude("aarch64/api.h");
        });
        return c.PQCLEAN_MLDSA87_AARCH64_crypto_sign_signature_ctx(sig, siglen, m, mlen, ctx, ctxlen, sk);
    } else {
        const c = @cImport({
            @cInclude("clean/api.h");
        });
        return c.PQCLEAN_MLDSA87_CLEAN_crypto_sign_signature_ctx(sig, siglen, m, mlen, ctx, ctxlen, sk);
    }
}
...
pub fn crypto_sign_verify_ctx(
    ...
    if (build_options.avx2) {
        ...
        return c.PQCLEAN_MLDSA87_AVX2_crypto_sign_verify_ctx(sig, siglen, m, mlen, ctx, ctxlen, pk);
    } else if (build_options.aarch64) {
        const c = @cImport({
            @cInclude("aarch64/api.h");
        });
        return c.PQCLEAN_MLDSA87_AARCH64_crypto_sign_verify_ctx(sig, siglen, m, mlen, ctx, ctxlen, pk);
    } else {
        const c = @cImport({
            @cInclude("clean/api.h");
        });
        return c.PQCLEAN_MLDSA87_CLEAN_crypto_sign_verify_ctx(sig, siglen, m, mlen, ctx, ctxlen, pk);
    }
}
...
pub fn crypto_sign_ctx(
    ...
    if (build_options.avx2) {
        ...
        return c.PQCLEAN_MLDSA87_AVX2_crypto_sign_ctx(sm, smlen, m, mlen, ctx, ctxlen, sk);
    } else if (build_options.aarch64) {
        const c = @cImport({
            @cInclude("aarch64/api.h");
        });
        return c.PQCLEAN_MLDSA87_AARCH64_crypto_sign_ctx(sm, smlen, m, mlen, ctx, ctxlen, sk);
    } else {
        const c = @cImport({
            @cInclude("clean/api.h");
        });
        return c.PQCLEAN_MLDSA87_CLEAN_crypto_sign_ctx(sm, smlen, m, mlen, ctx, ctxlen, sk);
    }
}
...
pub fn crypto_sign_open_ctx(
    ...
    if (build_options.avx2) {
        ...
        return c.PQCLEAN_MLDSA87_AVX2_crypto_sign_open_ctx(m, mlen, sm, smlen, ctx, ctxlen, pk);
    } else if (build_options.aarch64) {
        const c = @cImport({
            @cInclude("aarch64/api.h");
        });
        return c.PQCLEAN_MLDSA87_AARCH64_crypto_sign_open_ctx(m, mlen, sm, smlen, ctx, ctxlen, pk);
    } else {
        const c = @cImport({
            @cInclude("clean/api.h");
        });
        return c.PQCLEAN_MLDSA87_CLEAN_crypto_sign_open_ctx(m, mlen, sm, smlen, ctx, ctxlen, pk);
    }
}
...
pub fn crypto_sign_signature(
    ...
    if (build_options.avx2) {
        ...
        return c.PQCLEAN_MLDSA87_AVX2_crypto_sign_signature(sig, siglen, m, mlen, sk);
    } else if (build_options.aarch64) {
        const c = @cImport({
            @cInclude("aarch64/api.h");
        });
        return c.PQCLEAN_MLDSA87_AARCH64_crypto_sign_signature(sig, siglen, m, mlen, sk);
    } else {
        const c = @cImport({
            @cInclude("clean/api.h");
        });
        return c.PQCLEAN_MLDSA87_CLEAN_crypto_sign_signature(sig, siglen, m, mlen, sk);
    }
}
...
pub fn crypto_sign_verify(
    ...
    if (build_options.avx2) {
        ...
        return c.PQCLEAN_MLDSA87_AVX2_crypto_sign_verify(sig, siglen, m, mlen, pk);
    } else if (build_options.aarch64) {
        const c = @cImport({
            @cInclude("aarch64/api.h");
        });
        return c.PQCLEAN_MLDSA87_AARCH64_crypto_sign_verify(sig, siglen, m, mlen, pk);
    } else {
        const c = @cImport({
            @cInclude("clean/api.h");
        });
        return c.PQCLEAN_MLDSA87_CLEAN_crypto_sign_verify(sig, siglen, m, mlen, pk);
    }
}
...
pub fn crypto_sign(
    ...
    if (build_options.avx2) {
        ...
        return c.PQCLEAN_MLDSA87_AVX2_crypto_sign(sm, smlen, m, mlen, sk);
    } else if (build_options.aarch64) {
        const c = @cImport({
            @cInclude("aarch64/api.h");
        });
        return c.PQCLEAN_MLDSA87_AARCH64_crypto_sign(sm, smlen, m, mlen, sk);
    } else {
        const c = @cImport({
            @cInclude("clean/api.h");
        });
        return c.PQCLEAN_MLDSA87_CLEAN_crypto_sign(sm, smlen, m, mlen, sk);
    }
}
...
pub fn crypto_sign_open(
    ...
    if (build_options.avx2) {
        ...
        return c.PQCLEAN_MLDSA87_AVX2_crypto_sign_open(m, mlen, sm, smlen, pk);
    } else if (build_options.aarch64) {
        const c = @cImport({
            @cInclude("aarch64/api.h");
        });
        return c.PQCLEAN_MLDSA87_AARCH64_crypto_sign_open(m, mlen, sm, smlen, pk);
    } else {
        const c = @cImport({
            @cInclude("clean/api.h");
        });
        return c.PQCLEAN_MLDSA87_CLEAN_crypto_sign_open(m, mlen, sm, smlen, pk);
    }
}
...
test "byte lengths" {
    if (build_options.avx2) {
        ...
        try std.testing.expectEqual(SK_BYTE_LEN, c.PQCLEAN_MLDSA87_CLEAN_CRYPTO_SECRETKEYBYTES);
        try std.testing.expectEqual(PK_BYTE_LEN, c.PQCLEAN_MLDSA87_CLEAN_CRYPTO_PUBLICKEYBYTES);
        try std.testing.expectEqual(SIG_BYTE_LEN, c.PQCLEAN_MLDSA87_CLEAN_CRYPTO_BYTES);
        try std.testing.expect(std.mem.eql(u8, ALG_NAME, c.PQCLEAN_MLDSA87_CLEAN_CRYPTO_ALGNAME));
    } else if (build_options.aarch64) {
        const c = @cImport({
            @cInclude("aarch64/api.h");
        });
